...
function Portfolio(props) {
  ...
  async function fetchPortfolio() {
    ...
    if (userId.length > 0) {
      const ptfurl = `https://web-production-8952.up.railway.app/portfolio/${userId}`;
      ...
      if (ptfdata.status === 404) {
        ...
        for (const position of ptfdata.positions) {
          ...
          let stockurl = `https://web-production-8952.up.railway.app/stocks/${position.ticker}/latestdata`;
          let stockresponse = await fetch(stockurl, fetchData);
          let stockdata = await stockresponse.json();
          if (stockdata.last == null) {
            stockdata.last = stockdata.high;
          }
  ...
  return (
    <div className="portfoliopage">
      ...
      {portfolioErr && <div className="portfolioerr">{portfolioErr}</div>}
      {!portfolioErr && (
        <div className="portfoliocont">
          <div className="portfolioheader">
            <div className="portfoliovalue">
              Total Value: ${currValue.toFixed(2)}
            </div>
            <div className="realizedtot">
              Realized Gain/Loss: ${Number(realizedTot).toFixed(2)}
            </div>
            <div className="unrealizedtot">
              Unrealized Gain/Loss: ${(currValue - valueTotal).toFixed(2)}
            </div>
            <div className="portcreated">Portfolio started on: {createdAt}</div>
            <div className="portupdated">
              Last Activity in portfolio: {updatedAt}
            </div>
          </div>
                    ...
                    <Link to={`/stock/${position.ticker}`}>
                      <div className="position">
                        ...
                        <div className="currpricecont">
                          ...
                          <div className="currpriceamount">
                            ${position.currprice}
                          </div>
                        </div>
                        <div className="quantitycont">
                          <div className="quantitytext">Total shares</div>
                          <div className="quantityamount">
                            {position.quantity}
                          </div>
                        </div>
                        <div className="currvaluecont">
                          <div className="currvaluetext">
                            Today's total value
                          </div>
                          <div className="currvalueamount">
                            ${Number(position.currvalue).toFixed(2)}
                          </div>
                        </div>
                        <div className="unrealizedtotcont">
                          <div className="unrealizedtottext">
                            Unrealized gain/loss
                          </div>
                          <div className="unrealizedtotamount">
                            ${(position.currvalue - position.value).toFixed(2)}
                          </div>
                        </div>
                        <div className="realizedtotcont">
                          <div className="realizedtottext">
                            Realized gain/loss
                          </div>
                          <div className="realizedtotamount">
